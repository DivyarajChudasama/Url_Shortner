...
import (
	...
	"github.com/DivyarajChudasama/Url_Shortner/helpers"
)
...
func ShortenURL(c *fiber.Ctx) error {
	...
	r2 := database.CreateClient(1) // Implement rate limiting
	defer r2.Close()
	val, err := r2.Get(database.Ctx, c.IP()).Result()
	if err == redis.Nil {
		_ = r2.Set(database.Ctx, c.IP(), os.Getenv("API_QUOTA"), 10*60*time.Second).Err()
	} else {
